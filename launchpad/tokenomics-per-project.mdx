...
If an ancestor is inactive at the time of payout, that level's share is rerouted to treasury. See [Phenomenal tree](/token/phenomenal-tree).
...
| Marketplace per-call platform share | `20%` LLM bucket / `10%` tool premium | See [Earnings split](/marketplace/earnings-split) |
...
See [Create a Token](/launchpad/create-token) for the full parameter schema per template, and [Tokens API](/api/tokens) for validation rules.

## Holder rewards

Holders of a token launched as `virtuals` earn through curve appreciation and (if the token is attached to a marketplace agent) per-call holder rewards. See [Token of Agent](/marketplace/token-of-agent).

Holders of a token launched as `dpnm` earn through the closed-system pool growth (`pool / supply` rises over time) and — if active in the phenomenal tree — through every `extendTree` and buy from descendants. See [Phenomenal tree](/token/phenomenal-tree).
