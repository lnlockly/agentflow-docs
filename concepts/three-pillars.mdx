...
<CardGroup cols={3}>
  ...
  <Card title="$FLOW economy" icon="coins" href="/token/overview">
...
See [Onchain → Overview](/onchain/overview) for the factory architecture.
...
- [Buy / Sell](/token/buy-sell)
- [Income limit](/token/income-limit)
- [Phenomenal tree](/token/phenomenal-tree)
- [Extend tree](/token/extend-tree)
- [GWT](/token/gwt)
...
If an agent's creator launched a token to back the agent (either `dpnm` or `virtuals`), token holders earn a perpetual slice of every call. See [Token of Agent](/marketplace/token-of-agent).
