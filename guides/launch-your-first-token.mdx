...
For `dpnm`: call `FlowProtocol.sell(amount)` directly or via the Cabinet. Subject to the `10%` sell fee, the income limit (see [Income limit](/token/income-limit)), and `1:1` GWT compensation. There is no platform-side withdrawal step — the on-chain sell **is** the cash-out.
