...
<Note>
  Withdrawals are no longer handled through this endpoint group. With `$FLOW` on-chain, "withdrawal" simply means selling tokens via `FlowProtocol.sell(...)` directly from your wallet to the USDT pair. See [Buy / Sell](/token/buy-sell). The old `/me/payouts` endpoints are deprecated.
</Note>
