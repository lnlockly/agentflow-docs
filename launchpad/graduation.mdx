...
<Warning>
  Tokens launched via the `dpnm` template — including `$FLOW` itself — **do not graduate**. They live inside the protocol contract forever, with closed-system economics. See [Closed system](/concepts/closed-system).
</Warning>
...
On BSC testnet: [`0x24C8...4ED8`](https://testnet.bscscan.com/address/0x24C8C2039Ae23E81379a03FA3B297AD0d1474ED8). Routes through PancakeSwap V2 router. Base mainnet uses Uniswap V2 router; Base Sepolia uses the Sepolia v2 router. Address per chain in [Deployments](/onchain/deployments).
...
The closed system is the feature. See [Closed system](/concepts/closed-system).
