...
After `activate`, you have **30 days of tree active status** — meaning you can earn from descendants buying or extending. To stay active beyond 30 days, call [`extendTree`](/token/extend-tree).
...
| Distribute `10 USDT` to phenomenal tree ancestors | per-level payout, see [Phenomenal tree](/token/phenomenal-tree) |
...
| Burn income limit | see [Income limit](/token/income-limit) for the `<=` vs `>` cases |
...
This is the closed-system invariant. See [Closed system](/concepts/closed-system) for the formal statement.
...
See [FLOW Onchain API](/api/flow-onchain).
