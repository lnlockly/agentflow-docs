...
This template is used for **memecoin-style** or **agent-of-marketplace** tokens that want to graduate to a public DEX once they hit a fundraising threshold. The platform's own `$FLOW` token does **not** use this template — it uses [`dpnm`](/concepts/closed-system) instead.
...
See [Graduation](/launchpad/graduation) for parameters and timing.
...
See [Closed system](/concepts/closed-system) for the full `dpnm` philosophy.
...
Response includes `template` (`"virtuals"` or `"dpnm"`), `state`, `progressToGraduation` (`virtuals` only), `holdersCount`, and `lastTradePrice`. See [Tokens API](/api/tokens).
